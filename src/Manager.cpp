...
ghc::filesystem::path const& Manager::getNoahhDirectory() const {
    return m_noahhDirectory;
}
...
Result<> Manager::loadData() {
    ...
    wxRegKey key(wxRegKey::HKLM, REGKEY_NOAHH);
    if (key.HasValue(REGVAL_INSTALLDIR)) {
        wxString value;
        key.QueryValue(REGVAL_INSTALLDIR, value);
        m_noahhDirectory = value.ToStdWstring();
        m_noahhDirectorySet = true;
    } else {
        TCHAR pf[MAX_PATH];
        ...
        m_noahhDirectory = ghc::filesystem::path(pf) / "NoahhSDK";
        m_noahhDirectorySet = false;
    }
    ...
    parent = parent / "NoahhSDK";
    std::string noahhDirectory = "";
    if (ghc::filesystem::exists(parent)) {
        m_noahhDirectorySet = true;
        m_noahhDirectory = parent;
    }

    #else
    static_assert(false, "Implement Manager::LoadData!");
    // get Noahh dir location or set default
    #endif

    auto installJsonPath = m_noahhDirectory / INSTALL_DATA_JSON;
...
Result<> Manager::saveData() {
    ghc::filesystem::path installJsonPath = m_noahhDirectory / INSTALL_DATA_JSON;
    ...
    #ifdef _WIN32
    wxRegKey key(wxRegKey::HKLM, "Software\\NoahhSDK");
    if (!key.Create()) {
        return Err(
            "Unable to create Registry Key - "
            "the installer wont be able to uninstall Noahh!"
        );
    }
...
Result<> Manager::deleteData() {
    ...
    if (ghc::filesystem::exists(m_noahhDirectory)) {
        if (!ghc::filesystem::remove_all(m_noahhDirectory)) {
            return Err("Unable to delete Noahh directory");
        }
    }
    return Ok();
}
...
Result<> Manager::uninstallFrom(Installation const& inst) {
    ...
    if (ghc::filesystem::exists(path / "noahh")) {
        ghc::filesystem::remove_all(path / "noahh");
    }
    if (ghc::filesystem::exists(path / "XInput9_1_0.dll")) {
        ghc::filesystem::remove(path / "XInput9_1_0.dll");
    }
    if (ghc::filesystem::exists(path / "Noahh.dll")) {
        ghc::filesystem::remove(path / "Noahh.dll");
    }
    return Ok();
...
Result<> Manager::deleteSaveDataFrom(Installation const& inst) {
    ...
    path = path.parent_path() / ghc::filesystem::path(inst.m_exe.ToStdString()).replace_extension() / "noahh";
...
int Manager::doesDirectoryContainOtherMods(
    ...
    if (
        ghc::filesystem::exists(path / "Noahh.dll") ||
