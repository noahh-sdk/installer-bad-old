...
#define NOAHH_DIR "Noahh"
#define NOAHH_SUITE_ENV "NOAHH_SUITE"
...
Result<> Manager::addSuiteEnv() {
    ...
    if (!key.SetValue(NOAHH_SUITE_ENV, m_suiteDirectory.wstring())) {
        return Err("Unable to set " NOAHH_SUITE_ENV " environment variable");
    }
    SendMessageTimeout(
        HWND_BROADCAST,
        WM_SETTINGCHANGE,
...
void Manager::checkForUpdates(
    ...
    #ifdef _WIN32
    auto exe = (installation.m_path / "Noahh.dll").wstring();
...
void Manager::checkCLIForUpdates(
    ...
    #ifdef _WIN32
    auto exe = (m_binDirectory / "Noahh.exe").wstring();
...
ghc::filesystem::path Manager::getDefaultDataDirectory() const {
    ...
    ).parent_path() / NOAHH_DIR;
    
    #elif defined(__APPLE__)

    return "/Users/Shared/" NOAHH_DIR; 
...
Result<> Manager::loadData() {
    ...
    auto suite = getenv(NOAHH_SUITE_ENV);
    if (suite != nullptr) {
...
Result<> Manager::uninstallSuite() {
    try {
        if (
            ...
            return Err("Unable to delete the Noahh Suite directory");
        }
    } catch(std::exception& e) {
        return Err("Error deleting data: " + std::string(e.what()));
    }
