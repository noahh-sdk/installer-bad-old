...
void Manager::downloadCLI(
    ...
    this->webRequest(
        "https://api.github.com/repos/noahh-sdk/cli/releases/latest",
        "",
        false,
        errorFunc,
        nullptr,
...
ghc::filesystem::path Manager::getDefaultSDKDirectory() const {
    ...
    return ghc::filesystem::path(pf) / "NoahhSDK";
    
    #elif defined(__APPLE__)

    return "/Users/Shared/Noahh/SDK"; 
...
Result<> Manager::loadData() {
    ...
    if (key.Exists()) {
        ...
        if (key.HasSubKey(REGSUB_INSTALLATIONS)) {
            wxRegKey sub(wxRegKey::HKLM, REGKEY_NOAHH "\\" REGSUB_INSTALLATIONS);
            wxString subKey;
            long index = 0;
            for (
                ...
                wxRegKey instKey(
                    wxRegKey::HKLM,
                    REGKEY_NOAHH "\\" REGSUB_INSTALLATIONS "\\" + subKey
                );
                Installation inst;
...
Result<> Manager::saveData() {
    ...
    #define UHHH_WELP(err) \
        return Err(err " - the installer wont be able to uninstall Noahh!")

    wxRegKey key(wxRegKey::HKLM, REGKEY_NOAHH);
    if (!key.Create()) {
        UHHH_WELP("Unable to create " REGKEY_NOAHH);
    }
    if (m_sdkInstalled) {
        if (!key.SetValue(REGVAL_SDKDIR, m_sdkDirectory.wstring())) {
            UHHH_WELP("Unable to save " REGKEY_NOAHH "\\" REGVAL_SDKDIR);
        }
    }
    wxRegKey subKey(wxRegKey::HKLM, REGKEY_NOAHH "\\" REGSUB_INSTALLATIONS);
    if (!subKey.Create()) {
...
Result<> Manager::installSDK(
    ...
    std::thread t([this, branch, errorFunc, progressFunc, finishFunc]() -> void {
        ...
        auto lib = dlopen((m_sdkDirectory / "bin" / "libnoahhutils.dylib").string().c_str(), RTLD_LAZY);
        if (!lib) {
            throwError("Unable to load library");
        }
...
Result<> Manager::uninstallSDK() {
    if (
        ...
        return Err("Unable to delete the NoahhSDK directory");
    }
    #ifdef _WIN32
    wxRegKey key(wxRegKey::HKLM, "System\\CurrentControlSet\\Control\\Session Manager\\Environment");
