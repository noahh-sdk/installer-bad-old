...
void MainFrame::onDiscord(wxCommandEvent&) {
    wxLaunchDefaultBrowser("https://discord.gg/aKhJ7YyAk2");
}
...
void MainFrame::loaderUpdateWindow() {
    ...
    Manager::get()->downloadLoader(
        [](std::string const& error) -> void {
            wxMessageBox(
                "Error updating Noahh: " + error + ". Try "
...
MainFrame::MainFrame() : wxFrame(
    ...
    if (!res) {
        wxMessageBox(
            ...
            "The installer may be unable to uninstall Noahh!"
        );
    }
    ...
    auto bottomSizer = new wxStaticBoxSizer(
        new wxStaticBox(bottomControls, wxID_ANY, "(c) Noahh Team 2022 - " INSTALLER_VERSION),
        wxHORIZONTAL
    );
