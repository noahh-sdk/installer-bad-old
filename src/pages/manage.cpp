        ...
        if (GET_EARLIER_PAGE(ManageSelect)->updateCLI()) {
            Manager::get()->checkCLIForUpdates(
                ...
                [this](
                    ...
                    if (current < available) {
                        this->setText(m_nextInfo, "Press \"Next\" to update Noahh CLI.");
                        m_canContinue = true;
                        m_frame->updateControls();
                    } else {
            ...
            Manager::get()->checkForUpdates(
                ...
                [this](
                    ...
                    if (current < available) {
                        this->setText(m_nextInfo, "Press \"Next\" to update Noahh.");
                    } else {
                        this->setText(m_nextInfo,
                            "You are up-to-date! :)\n"
                            "Press \"Next\" to reinstall Noahh anyway, "
        ...
        if (!GET_EARLIER_PAGE(ManageSelect)->updateCLI()) {
            ...
            if (inst.m_branch == DevBranch::Stable) {
                this->addText(
                    ...
                    "to use the beta version of the Noahh loader? "
                ...
                this->addText(
                    "Would you like to switch this installation "
                    "to use the stable version of the Noahh loader? "
        ...
        if (GET_EARLIER_PAGE(ManageSelect)->updateCLI()) {
            Manager::get()->downloadCLI(
                ...
                [this](wxWebResponse const& wres) -> void {
                    ...
                    if (!installRes) {
                        wxMessageBox(
                            "Error updating Noahh CLI: " + installRes.error() + ". Try "
...
public:
    PageManageUpdate(MainFrame* frame) : Page(frame) {
        if (GET_EARLIER_PAGE(ManageSelect)->updateCLI()) {
            this->addText("Updating Noahh CLI");
        } else {
