...
public:
    PageUninstallSelect(MainFrame* frame) : Page(frame) {
        this->addText("Please select which parts of Noahh to uninstall.");
...
protected:
    ...
    void enter() override {
            ...
            if (GET_EARLIER_PAGE(UninstallSelect)->shouldUninstall(inst)) {
                ...
                if (GET_EARLIER_PAGE(UninstallDeleteData)->shouldDeleteData()) {
                    ...
                    if (!dr) {
                        ...
                        try {
                            wxMessageBox(
                                "Unable to delete Noahh save data from " + inst.m_path.string() + ": " +
                            ...
                            wxMessageBox(
                                ...
                                "owner for help. Otherwise, contact the Noahh "
        ...
        if (GET_EARLIER_PAGE(UninstallSelect)->shouldUninstallSuite()) {
            ...
            if (!sr) {
                wxMessageBox(
                    "Unable to uninstall the Noahh SDK: " + sr.error() +
...
public:
    PageUninstallFinished(MainFrame* frame) : Page(frame) {
        ...
        this->addText(
            "If you find parts of Noahh still lurking "
            "around, contact the Noahh Development Team "
            "for help."
        );
