...
protected:
    ...
    void onBrowse(wxCommandEvent&) {
        wxDirDialog dlg(
            this, "Select Directory for the Noahh SDK", 
...
public:
    PageDevInstallSelectSDK(MainFrame* parent) : Page(parent) {
        this->addText("Please select where to install the Noahh SDK.");
...
public:
    PageDevInstallBranch(MainFrame* frame) : Page(frame) {
        ...
        this->addText(
            ...
            "greatest features of Noahh, whereas "
...
public:
    PageDevInstallAddToPath(MainFrame* frame) : Page(frame) {
        this->addText(
            "Would you like to add the Noahh CLI to your "
            "PATH environment variable? This will make "
            "Noahh usable from the command line anywhere "
            "on your computer."
        );
    ...
    void enter() override {
        Manager::get()->downloadCLI(
            [this](std::string const& str) -> void {
                wxMessageBox(
                    "Error downloading the Noahh CLI: " + str + 
            ...
            [this](std::string const& text, int prog) -> void {
                this->setText(m_status, "Downloading Noahh CLI: " + text);
            ...
            [this](wxWebResponse const& wres) -> void {
                ...
                if (!installRes) {
                    wxMessageBox(
                        "Error installing Noahh CLI: " + installRes.error() + ". Try "
                    ...
                    auto res = Manager::get()->installSuite(
                        ...
                        [this](std::string const& err) -> void {
                            wxMessageBox(
                                "Error installing the Noahh SDK: " + err + 
                                ". Try again, and if the problem persists, contact "
                                "the Noahh Development team for more help.",
                                "Error Installing",
                                wxICON_ERROR
                        ...
                        [this]() -> void {
                            if (GET_EARLIER_PAGE(DevInstallAddToPath)->shouldAddToPath()) {
                                ...
                                if (!res) {
                                    wxMessageBox(
                                        "Error adding Noahh CLI to Path: " + res.error(),
                                        "Error Installing",
                                        wxICON_ERROR
                                    );
                                }
...
public:
    PageDevInstall(MainFrame* frame) : Page(frame) {
        this->addText("Installing the Noahh SDK...");
...
protected:
    ...
    void onDocs(wxCommandEvent&) {
        wxLaunchDefaultBrowser("https://noahh-sdk.github.io/docs");
    }

public:
    PageDevInstallFinished(MainFrame* frame) : Page(frame) {
        this->addText(
            "The Noahh SDK has been installed! "
            "Happy modding :)"
        );
