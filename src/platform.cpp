...
std::string MainFrame::SaveInstallInfo() {
    auto path = std::filesystem::absolute("NoahhInstallationInfo.json");
...
OtherModLoaderInfo MainFrame::DetectOtherModLoaders() {
    ...
    if (std::filesystem::exists(m_installingTo / "Noahh.dll")) {
...
bool MainFrame::UninstallNoahh(Installation const& inst) {
    ...
    if (std::filesystem::exists(path / "noahh")) {
        std::filesystem::remove_all(path / "noahh");
    }
    if (std::filesystem::exists(path / "XInput9_1_0.dll")) {
        std::filesystem::remove(path / "XInput9_1_0.dll");
    }
    if (std::filesystem::exists(path / "Noahh.dll")) {
        std::filesystem::remove(path / "Noahh.dll");
    }
    return true;
    #else
...
bool MainFrame::DeleteSaveData(Installation const& inst) {
    ...
    path = path.parent_path() / std::filesystem::path(inst.m_exe).replace_extension() / "noahh";
    if (std::filesystem::exists(path)) {
