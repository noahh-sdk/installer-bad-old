...
void MainFrame::LoadInstallInfo() {
    ...
    if (ghc::filesystem::exists(parent)) {
        m_firstTime = false;
        installInfo = (parent / "NoahhInstallationInfo.json").string();
    }
    #else
...
std::string MainFrame::SaveInstallInfo() {
    #ifdef _WIN32
    auto path = ghc::filesystem::absolute("NoahhInstallationInfo.json");
    #elif defined(__APPLE__)
    FSRef ref;
    OSType folderType = kApplicationSupportFolderType;
    char pth[PATH_MAX];
    ...
    path = path / "NoahhInstallationInfo.json";
    #endif
...

    {
        ...
        if (!file.is_open()) return "Unable to write installation data - the installer wont be able to uninstall Noahh!";
        file << json.dump(4);
    }
    ...
    wxRegKey key(wxRegKey::HKLM, "Software\\NoahhSDK");
    if (!key.Create()) {
        return "Unable to create Registry Key - the installer wont be able to uninstall Noahh!";
    }
    if (!key.SetValue("InstallInfo", path.wstring())) {
        return "Unable to save Registry Key - the installer wont be able to uninstall Noahh!";
    }
    #endif
    return "";
}
...
void MainFrame::DeleteInstallInfo() {
    ...
    ghc::filesystem::path path(pth);
    path = path / "NoahhSDK" / "NoahhInstallationInfo.json";
    if (ghc::filesystem::exists(path)) {
        ghc::filesystem::remove(path);
    }
    #else
...
OtherModLoaderInfo MainFrame::DetectOtherModLoaders() {
    ...
    if (ghc::filesystem::exists(m_installingTo / "Noahh.dll")) {
        found.m_hasNoahh = true;
    }
...
std::string MainFrame::InstallNoahh(wxString const& file) {
    #ifdef _WIN32
...
bool MainFrame::UninstallNoahh(Installation const& inst) {
    #ifdef _WIN32
...
bool MainFrame::DeleteSaveData(Installation const& inst) {
    ...
    path = path.parent_path() / ghc::filesystem::path(inst.m_exe).replace_extension() / "noahh";
    if (ghc::filesystem::exists(path)) {
