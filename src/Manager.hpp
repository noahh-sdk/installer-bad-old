...
/**
 * Represents an installation of Noahh 
...
struct Installation {
     ...
     * dir under %localappdata%/${m_exe}/noahh on 
     * Windows.
     */
    wxString m_exe;
    DevBranch m_branch;
...
class NoahhInstallerApp;

namespace cli {
    using ProgressCallback = void(__stdcall*)(const char*, int);
    using noahh_install_noahh = const char*(__cdecl*)(const char*, bool, bool, ProgressCallback);
    using noahh_install_suite = const char*(__cdecl*)(const char*, bool, ProgressCallback);
}
...
protected:
    ...
    friend class NoahhInstallerApp;
...
public:
    ...
    bool isNoahhUtilsInstalled() const;

    Result<> installNoahhFor(
    ...
    Result<> uninstallNoahhFrom(Installation const& installation);
